...
class Assistente:
    def __init__(self, root: tk.Tk):
        ...
        self.AIML = carica_aiml(self.cfg.get("lingua", "it"))
        self.stato     = "idle"
        self.dati_temp = {}
        ...
        # AIML — carica regole per la lingua corrente
        _aiml_dati = carica_aiml(self.cfg.get("lingua", "it"))
        self._aiml_regole:   list = _aiml_dati[0]   # lista regole ordinate
        self._aiml_ignora:   str  = _aiml_dati[1]   # caratteri da ignorare
        self._aiml_contesto: str | None = None       # trigger ultima regola (followup)
    ...
    def _cmd_lingua(self, parsed: dict):
        ...
        self._aiml_ignora   = _aiml_dati[1]
        self._aiml_contesto = None
        self.root.title(self._t("titolo_finestra", nome_avatar=self.cfg["nome_avatar"]))
    ...
    # ------------------------------------------------------------------
    # AIML — risposta e gestione contesto
    # ------------------------------------------------------------------
    def _aiml_rispondi(self, regola: dict):
        """
        Elabora una regola AIML matchata:
        - espande i segnaposto nella risposta
        - mostra l'avatar indicato (o quello di default)
        - aggiorna il contesto followup
        """
        risposta = self._espandi_segnaposto(regola.get("risposta", ""))
        self._scrivi_risposta(risposta)

        # Avatar: se specificato usa quello, altrimenti random
        avatar = regola.get("avatar")
        if avatar:
            # Se non ha estensione, il motore avatar prova .jpg e poi .mp4
            self._mostra_avatar(avatar)
        else:
            self._mostra_avatar(avatar_random(self.cfg))

        # Aggiorna contesto per il followup
        # Se la regola ha un followup, la prossima frase viene controllata lì
        if regola.get("followup") and not regola.get("_da_followup"):
            self._aiml_contesto = regola.get("trigger")
        else:
            # Il followup è stato consumato, azzera il contesto
            self._aiml_contesto = None

    def _non_capito(self):
        self._mostra_avatar("triste")
        self._scrivi_risposta(self._t("non_capito"))
        self._aiml_contesto = None   # azzera contesto in caso di comando non capito

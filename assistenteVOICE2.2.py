"""
Personal Desktop Asssistant v2.2 VOICE + multiLANGUAGE
Assistente Personale v.2.2 VOICE + multiLINGUA

    STT (microphone) multiplatform
    TTS (voice) S.O. Windows only -> uses win32com
        for TTS multiplatform, change win32com/SAPI with pyttsx3
"""
...
# ---------------------------------------------------------------------------
# STT / TTS  — importazioni opzionali (graceful degradation)
# ---------------------------------------------------------------------------
...
try:
    ...
    import numpy as np
    # Verifica runtime: su Linux PyInstaller, sounddevice può importarsi
    # ma fallire con OSError se PortAudio non è disponibile nel bundle
    _devs = sd.query_devices()
    STT_DISPONIBILE = True
except ImportError:
    pass   # libreria mancante
except Exception as _e:
    print(f"[STT] Non disponibile: {_e}")
...
MEM_FILE     = INTERNAL_DIR / "memoria.json"
CFG_FILE     = INTERNAL_DIR / "configurazione.json"
    ...
    if CFG_FILE.exists():
        try:
            ...
            for k, v in DEFAULT_CONFIG.items():
                cfg.setdefault(k, v)
            # Rimuove chiavi obsolete migrate nei file lingua
            cfg.pop("lingua_stt", None)
            return cfg
        except Exception:
            pass
...
def carica_lingua(codice: str) -> dict:
    ...
    # Fallback: cerca nella stessa cartella dell'eseguibile/script
...
_LANG_IT_FALLBACK = {
    "btn_invia": "Invia", "titolo_finestra": "Assistente – {nome_avatar}",
    "label_comandi": "COMANDI", "label_shortcut": "SHORTCUT",
    "panel_dammi": "dammi ...", "panel_apri": "apri ...",
    "panel_cerca": "cerca ...", "panel_elenca": "elenca memoria",
    "panel_aiuto": "aiuto", "panel_esci": "esci",
    "panel_sep_memoria": "── GESTISCI MEMORIA ──", "panel_impara": "impara",
    "panel_ricorda": "ricorda ...", "panel_ricorda_img": "ricorda immagine",
    "panel_modifica": "modifica ...", "panel_elimina": "elimina ...",
    "panel_copia": "copia ...",
    ...
    "apri_subito": "Apro subito", "apri_cosa": "Che cosa devo aprire?",
    ...
    "cerca_cosa": "Cosa devo cercare?", "cerca_non_trovato": "Nessun risultato per '{q}'.",
    "cerca_trovato": "Ho trovato {n} risultato/i per '{q}':",
    "elimina_cosa": "Cosa devo eliminare?", "elimina_non_trovato": "Non ho trovato '{q}'.",
    "elimina_conferma": "Vuoi eliminare:\n{entry}\nScrivi 'sì' per confermare.",
    "elimina_ok": "'{nome}' eliminato.", "elimina_annullato": "Eliminazione annullata.",
    "elimina_si": ["sì", "si", "s", "yes"],
    "modifica_cosa": "Quale voce devo modificare?", "modifica_non_trovato": "Non ho trovato '{q}'.",
    ...
    "modifica_ok": "Modificato! '{campo}' = {valore}", "modifica_errore": "Errore: voce non trovata.",
    "elenca_vuota": "La memoria è vuota.", "elenca_intestazione": "Ho {n} voce/i in memoria:",
    "copia_cosa": "Cosa devo copiare?", "copia_non_trovato": "Non ho trovato '{q}'.",
    "copia_ok": "Copiato:\n  {dati}",
    "ricorda_chiedi_nome": "Che cosa devo ricordare? Dimmi il nome:",
    "ricorda_chiedi_dati": "Dimmi i dati per '{nome}':", "ricorda_ok": "Fatto! '{nome}' salvato.",
    "ricorda_img_titolo": "Seleziona immagine", "ricorda_img_nessuna": "Nessuna immagine selezionata.",
    "ricorda_img_nome": "Nome per l'immagine?", "ricorda_img_tag": "Descrizione (tag)?",
    ...
    "impara_annullato": "Inserimento annullato.", "impara_obbligatorio": "Il campo '{campo}' è obbligatorio.",
    "impara_salvato": "Salvato! '{nome}'.\n\nAltro elemento? (sì / no)",
    "impara_completato": "Inserimento completato.", "impara_si": ["sì", "si", "s", "yes"],
    "impara_fine": "fine", "impara_salta": "salta",
    ...
    "aiuto_testo": "Comandi: ricorda · impara · apri · dammi · cerca · elenca · modifica · elimina · copia · lingua · esci · aiuto",
    "non_capito": "Non ho capito. Parole chiave: ricorda · impara · apri · dammi · cerca · elenca · modifica · elimina · copia · esci · aiuto",
    "lingua_ok": "Lingua impostata su: {lingua}", "lingua_non_trovata": "File lingua '{file}' non trovato.",
    "lingua_uso": "Uso: lingua <codice>  (es: lingua en)",
    "stt_nessun_audio": "Nessun audio rilevato.", "stt_non_capito": "Non ho capito, riprova.",
    "stt_errore": "Errore STT: {err}", "stt_mic_errore": "Microfono non disponibile: {err}",
    "shortcut_nessuno": "Nessuno shortcut configurato.",
    "label_lingua": "LINGUA", "lingua_nessuna": "Nessun file lingua\ntrovato in _dati/",
    "lingua_stt": "it-IT",
}
    ...
    if MEM_FILE.exists():
        try:
            ...
            with open(MEM_FILE, "w", encoding="utf-8") as f:
                json.dump([], f)
            print(f"[AVVISO] memoria.json corrotto — backup in {backup.name}.")
            return []
    return []
...
COMANDI = ["ricorda", "apri", "dammi", "cerca", "elimina", "modifica",
           "elenca", "aiuto", "copia", "impara", "esci", "lingua"]
...
def estrai_soggetto(testo: str, nome_utente: str) -> tuple:
    ...
    # Riconosce un'unica parola con iniziale maiuscola in fondo al testo
    m = re.search(r'\b([A-Z][a-z]{2,})\s*$', testo)
    if m:
        nome_proprio = m.group(1)
        # Evita di confonderlo col nome utente o con parole comuni
...
def parse_comando(testo: str, nome_utente: str, alias_comandi: dict = None) -> dict:
    testo = testo.strip()
    parole = testo.split()
    if not parole:
...
class Assistente:
    def __init__(self, root: tk.Tk):
        ...
        self._history_idx: int   = -1   # -1 = nessuna navigazione attiva
        ...
        # STT
        self._stt_ascolto    = False   # True mentre il microfono è aperto
        self._stt_recognizer = sr.Recognizer() if STT_DISPONIBILE else None
        self._STT_SAMPLERATE  = 16000   # Hz ottimale per Google Speech
        self._STT_MAX_SECONDI = 10      # timeout massimo assoluto
        self._STT_SILENZIO_S  = 1.2     # secondi di silenzio per fermarsi
        self._STT_SOGLIA      = 200     # ampiezza minima per considerare "voce"
    ...
    def _lingue_disponibili(self) -> list:
        ...
        Restituisce lista di tuple (codice, etichetta) ordinate per codice.
        Deduplica se lo stesso file esiste in entrambe le cartelle.
        """
        # Mappa codice → nome leggibile (estendibile)
        NOMI_LINGUA = {
            "it": "Italiano 🇮🇹",
            "en": "English 🇬🇧",
            "fr": "Français 🇫🇷",
    ...
    def _mostra_immagine(self, path: Path):
        try:
            ...
            # ImportError (Pillow mancante), errori di decodifica,
            # o problemi di inizializzazione in ambienti frozen (PyInstaller Linux)
    ...
    def _placeholder_avatar(self, nome: str):
        ...
        self.avatar_label.configure(
            image="", text=nome.upper().replace(".MP4", ""),
            bg=col, fg="#1e1e2e",
    ...
    def _riproduci_video(self, path: Path, callback=None):
        try:
            ...
            while True:
                ...
                if not ret:
                    break
                # cv2 legge in BGR, converti in RGB
            ...
            # Salva sull'istanza per evitare garbage collection
    ...
    def _tts_worker(self):
        """Thread persistente TTS via Windows SAPI asincrono — interrompibile."""
        try:
            import pythoncom
            pythoncom.CoInitialize()
            speaker = win32com.client.Dispatch("SAPI.SpVoice")
            # Cerca voce nella lingua configurata
            lingua_stt = self.cfg.get("lingua", "it")
            voci = speaker.GetVoices()
            for i in range(voci.Count):
                v = voci.Item(i)
                desc = v.GetDescription().lower()
                vid  = v.Id.lower()
                if lingua_stt == "it" and ("italian" in desc or "ital" in vid):
                    speaker.Voice = v
                    break
                elif lingua_stt == "en" and ("english" in desc or "en-" in vid):
                    speaker.Voice = v
                    break
                elif lingua_stt in desc or lingua_stt in vid:
                    speaker.Voice = v
                    break
            speaker.Rate = 1
        except Exception as e:
        ...
        SVSFlagsAsync = 1   # costante SAPI per speak asincrono
        SPRS_SPEAKING  = 2   # stato "sta parlando"
        ...
        while True:
            try:
                ...
                # Interrompe immediatamente eventuale lettura in corso
                try:
                    speaker.Speak("", SVSFlagsAsync | 2)  # SVSFPurgeBeforeSpeak=2
                except Exception:
                    pass

                if not self._tts_attivo or not testo:
                    continue

                speaker.Speak(testo, SVSFlagsAsync)

                # Attende la fine, ma controlla la coda ogni 100ms
                while speaker.Status.RunningState == SPRS_SPEAKING:
                    # Nuovo messaggio in arrivo o mute attivato → interrompi
                    if not self._tts_queue.empty() or not self._tts_attivo:
                        speaker.Speak("", SVSFlagsAsync | 2)
                        break
                    import time
                    time.sleep(0.1)
    ...
    def _toggle_tts(self):
        ...
        if self._tts_attivo:
            ...
            # Sveglia il worker subito così rileva _tts_attivo=False e interrompe
            self._tts_queue.put("")
    ...
    def _toggle_ascolto(self):
        if self._stt_ascolto:
            return   # già in ascolto, ignora doppio click
    ...
    def _ascolta_thread(self):
        ...
        errore = None
        try:
            blocco      = int(self._STT_SAMPLERATE * 0.1)   # 100ms per blocco
            ...
            with sd.InputStream(samplerate=self._STT_SAMPLERATE,
                ...
                for _ in range(max_blocchi):
                    ...
                    ampiezza  = int(np.abs(chunk).mean())
                    ...
                    if ampiezza > self._STT_SOGLIA:
                        ...
                        if blocchi_silenzio >= sil_blocchi:
                            break   # silenzio rilevato → fine frase
            ...
            if not parlato_iniziato:
                ...
                testo     = self._stt_recognizer.recognize_google(
                    audio, language=self.L.get("lingua_stt", "it-IT"))
    ...
    def _dopo_ascolto(self, testo, errore):
        """Chiamato dal mainloop dopo che il thread STT ha finito."""
    ...
    # CRONOLOGIA
    # ------------------------------------------------------------------
        """Naviga indietro nella cronologia comandi (freccia ↑)."""
    ...
    def _history_giu(self):
        """Naviga avanti nella cronologia comandi (freccia ↓)."""
        if self._history_idx == -1:
            return
    ...
    def _on_invia(self):
        ...
        if not testo:
            return
        # Salva nella cronologia (evita duplicati consecutivi)
        if not self._history or self._history[-1] != testo:
            self._history.append(testo)
        self._history_idx = -1   # reset navigazione
        self._scrivi_utente(testo)
        self.input_var.set("")
        self._gestisci_input(testo)

    def _gestisci_input(self, testo: str):
        # Parola chiave per interrompere la sintesi vocale in corso
        if testo.strip().lower() in ("ok", "stop", "basta", "silenzio",
                                     "quiet", "enough") and self._tts_queue:
            self._tts_queue.put("")   # sveglia il worker → interrompe la lettura
            return
        ...
        for trigger, risposta in risposte_fisse.items():
            if testo_lower == trigger.lower():
                testo_risposta = risposta.replace(
        ...
        parsed = parse_comando(testo, self.cfg["nome_utente"], alias_merged)
    ...
    def on_close(self):
        """Punto di ingresso pubblico per la chiusura (WM_DELETE_WINDOW)."""
        self._cmd_esci()

    # ---- ESCI ----
    def _cmd_esci(self, parsed=None):
        # pylint: disable=unused-argument
        # Evita doppia chiamata (es. comando esci + click X)
        ...
        # Frase finale nel log
        self._scrivi_risposta(frase)

        # Frase finale come label sotto l'avatar
        ...
        def _chiudi():
            """Distrugge la finestra in modo sicuro."""
            if self._video_after_id:
                try:
                    self.root.after_cancel(self._video_after_id)
                except Exception:
        ...
        if path_video.exists():
            # Riproduce il video e chiude subito alla fine
            self._riproduci_video(path_video, callback=_chiudi)
        else:
            self._placeholder_avatar(avatar_fin.replace(".mp4", ""))
            # Nessun video: chiude dopo 2 secondi per mostrare la frase
    ...
    def _impara_prompt(self, campo: str) -> str:
        """Restituisce il prompt localizzato per il campo dato."""
    ...
    def _impara_campo(self, campo: str, valore: str):
        ...
        if t.lower() == self._t("impara_salta"):
            ...
            # alias e tag saltati → non aggiunti
        else:
            if campo == "tag":
    ...
    def _cmd_apri(self, parsed: dict):
        ...
        # Mostra il link completo nel log, parla solo col nome
        self.output.configure(state=tk.NORMAL)
    ...
    def _cmd_lingua(self, parsed: dict):
        """Cambia la lingua a runtime e ricostruisce il pannello."""
        ...
        self.root.title(self._t("titolo_finestra", nome_avatar=self.cfg["nome_avatar"]))
        # Ricostruisce il pannello con le nuove etichette

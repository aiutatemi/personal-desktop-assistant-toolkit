...
# AIML Parser (opzionale — richiede aiml_parser.py nella stessa cartella)
AIML_DISPONIBILE = False
try:
    from aiml_parser import AIMLParser
    AIML_DISPONIBILE = True
    print("[AIML] aiml_parser.py caricato")
except ImportError:
    print("[AIML] aiml_parser.py non trovato — motore AIML disabilitato")

# AI Integration (opzionale)
AI_DISPONIBILE = False
genai = None   # inizializzato qui; assegnato sotto se il modulo è disponibile
try:
...
_LANG_IT_FALLBACK = {
    ...
    # Cerca media in asset/
    "cerca_media_immagini":     "immagini",
    "cerca_media_musica":       "musica",
    "cerca_media_video":        "video",
...
class Assistente:
    ...
    def _tts_worker(self):
        ...
        while True:
            ...
            try:
                ...
                def on_word(name, location, length):  # pylint: disable=unused-argument
                ...
                try:
                    pyttsx3._activeEngines.clear()  # pylint: disable=protected-access
                except Exception:
                    pass
    ...
    def _costruisci_ui(self):
        ...
        # ── Header: nome programma + link ──────────────────────────────
        frame_header = tk.Frame(self.frame_chat, bg="#181825")
        frame_header.pack(fill=tk.X, padx=0, pady=0)

        tk.Label(frame_header, text=APP_NAME,
                 bg="#181825", fg="#89b4fa",
                 font=("Segoe UI", 11, "bold"),
                 padx=12, pady=6).pack(side=tk.LEFT)

        if APP_URL:
            lbl_link = tk.Label(frame_header, text="🌐",
                                bg="#181825", fg="#6c7086",
                                font=("Segoe UI", 12),
                                cursor="hand2", padx=6, pady=4)
            lbl_link.pack(side=tk.LEFT)
            lbl_link.bind("<Button-1>", lambda e: self._apri_url(APP_URL))
            lbl_link.bind("<Enter>",    lambda e: lbl_link.configure(fg="#94e2d5"))
            lbl_link.bind("<Leave>",    lambda e: lbl_link.configure(fg="#6c7086"))

        # Frame contenitore avatar + indicatore sorgente
        self._frame_avatar_area = tk.Frame(self.frame_chat, bg="#1e1e2e")
        self._frame_avatar_area.pack(pady=(16, 0))

        # Indicatore sorgente risposta (in basso a sinistra rispetto all'avatar)
        self._frame_sorgente = tk.Frame(self._frame_avatar_area, bg="#1e1e2e")
        self._frame_sorgente.pack(anchor="w", padx=4, pady=(0, 2))
        self._lbl_sorgente_pallino = tk.Label(
            self._frame_sorgente, text="●",
            bg="#1e1e2e", fg="#6c7086",
            font=("Segoe UI", 11)
        )
        self._lbl_sorgente_pallino.pack(side=tk.LEFT)
        self._lbl_sorgente_testo = tk.Label(
            self._frame_sorgente, text="—",
            bg="#1e1e2e", fg="#6c7086",
            font=("Segoe UI", 8)
        )
        self._lbl_sorgente_testo.pack(side=tk.LEFT, padx=(3, 0))

        self.avatar_label = tk.Label(self._frame_avatar_area, bg="#1e1e2e",
                                     width=300, height=300)
        self.avatar_label.pack(pady=(0, 4))
    ...
    def _localizza_comando(self, cmd_interno: str, con_spazio: bool) -> str:
        """
        Restituisce la parola chiave localizzata per un comando interno.
        Cerca in questo ordine:
          1. alias_comandi del file lingua attivo (lang_XX.json)
          2. alias_comandi di config.json
          3. Il comando interno stesso (fallback italiano)
        Se con_spazio=True aggiunge uno spazio finale (il pannello aspetta argomento).
        Caso speciale: "__ricorda_img__" → frase completa localizzata.
        """
        if cmd_interno == "__ricorda_img__":
            # Frase completa: usa la prima parola di "ricorda" + " questa immagine"
            # tradotta per la lingua corrente
            parola_ricorda = self._localizza_comando("ricorda", False)
            return f"{parola_ricorda} questa immagine"

        # Cerca negli alias della lingua attiva: il primo alias È la forma localizzata
        alias_lingua = self.L.get("alias_comandi", {})
        if cmd_interno in alias_lingua:
            lista = alias_lingua[cmd_interno]
            if lista:
                parola = lista[0]
                return parola + (" " if con_spazio else "")

        # Fallback: alias da config.json
        alias_cfg = self.cfg.get("alias_comandi", {})
        if cmd_interno in alias_cfg:
            lista = alias_cfg[cmd_interno]
            if lista:
                parola = lista[0]
                return parola + (" " if con_spazio else "")

        # Fallback finale: comando interno italiano
        return cmd_interno + (" " if con_spazio else "")
    ...
    # OUTPUT
    # ------------------------------------------------------------------
    def _aggiorna_sorgente(self, sorgente: str):
        """
        Aggiorna l'indicatore sorgente risposta.
        sorgente: "memoria" | "aiml" | "ai" | "sistema"
        """
        # Controllo di sicurezza: i widget potrebbero non essere ancora creati
        ...
        self._lbl_sorgente_testo.configure(fg=colore, text=etichetta)
    ...
    def _gestisci_input(self, testo: str):
        ...
        # ── 4. AIML / AI secondo la modalita' configurata ───────────────────
        modalita = self.cfg.get("risposta_config", {}).get("modalita", "aiml_then_ai")
        ai_cfg   = self.cfg.get("ai_config", {})
        ai_attiva = (ai_cfg.get("enabled", False) and self._ai_client)
        ...
        # aiml_only o aiml_then_ai: tenta prima AIML
    ...
    def _gestisci_stati_dialogo(self, testo: str):
        ...
        if self.stato == "attesa_conferma_apri":
            self._conferma_apri(testo); return
        if self.stato == "attesa_conferma_dammi":
    ...
    # ------------------------------------------------------------------
    # COMANDO CONFIGURA — dinamico da config.json
    # ------------------------------------------------------------------

    # Chiavi escluse dal wizard (troppo complesse per editing inline)
    _CONFIGURA_BLACKLIST = {"shortcut"}

    def _configura_chiavi_piatte(self) -> list:
        """
        Genera dinamicamente l'elenco delle chiavi configurabili leggendo
        config.json.  Restituisce una lista di tuple (chiave_dotted, valore).
        Le chiavi in _CONFIGURA_BLACKLIST vengono saltate.
        Dizionari annidati vengono espansi con notazione dot (es. tts_config.rate).
        Liste semplici (non di dict) vengono incluse come stringhe CSV.
        Liste di dict (es. shortcut) vengono escluse in automatico.
        """
        def _espandi(prefisso, obj):
            risultati = []
            if isinstance(obj, dict):
                for k, v in obj.items():
                    chiave = f"{prefisso}.{k}" if prefisso else k
                    radice = chiave.split(".")[0]
                    if radice in self._CONFIGURA_BLACKLIST:
                        continue
                    risultati.extend(_espandi(chiave, v))
            elif isinstance(obj, list):
                # Escludi liste di dict (es. shortcut già bloccato sopra,
                # ma potrebbe esserci altro); includi liste semplici come CSV
                if obj and isinstance(obj[0], dict):
                    pass  # salta
                else:
                    risultati.append((prefisso, ", ".join(str(x) for x in obj)))
            else:
                risultati.append((prefisso, obj))
            return risultati

        return _espandi("", self.cfg)
    ...
    def _configura_wizard_inizio(self):
        """Avvia la configurazione guidata, leggendo le chiavi da config.json."""
        self.stato = "configura_wizard_attesa"
        self.dati_temp = {}
        self._configura_queue = self._configura_chiavi_piatte()
        intro = self._t("configura_intro") + "\n"
        for chiave, valore in self._configura_queue:
            intro += f"\n  • {chiave} = {valore!r}"
        intro += f"\n\n{self._t('configura_chiedi_valore', param='parametro (es: tts_config.rate)')}"
        self._scrivi_risposta(intro)

    def _configura_prossimo(self):
        """Chiede il valore per il prossimo parametro nella coda."""
        if not self._configura_queue:
            self._configura_fine()
            return
        chiave, valore_attuale = self._configura_queue[0]
    ...
    def _set_nested_config(self, key: str, value):
        """Imposta un valore annidato nel config, con conversione automatica di tipo."""
        parts = key.split(".")
        # Determina il tipo attuale per la conversione
        tipo = type(self._get_nested_config(key))
        if tipo == int:
            try:
                value = int(value)
            ...
            value = str(value).lower() in ("true", "1", "sì", "si", "yes", "on")
        elif tipo == list:
            # Lista semplice: ricostruisci da CSV
    ...
    def _configura_step(self, testo: str):
        """Router per tutti gli stati configura_*."""
        ...
        # Parole di uscita esplicite
        parole_stop = {"stop", "annulla", "fine", "esci", "quit", "cancel"}
        if testo.strip().lower() in parole_stop:
            self._configura_annulla()
            return

        # Conferma interruzione (stato speciale)
        if self.stato == "configura_conferma_stop":
            if testo.strip().lower() in self._t("elimina_si"):
                self._configura_annulla()
            else:
                # Ripristina lo stato precedente e richiedi il parametro corrente
        ...
        if self.stato == "configura_wizard_attesa":
            ...
            # Cerca corrispondenza parziale tra le chiavi disponibili
            trovata = None
            for chiave, _ in self._configura_queue:
                if chiave_scelta == chiave.lower() or chiave.lower().endswith(chiave_scelta):
                    trovata = chiave
                    break
            if trovata:
                valore_attuale = self._get_nested_config(trovata)
                self.stato = "configura_attesa_valore"
                ...
                self._scrivi_risposta(
                    self._t("configura_parametro_non_trovato", param=chiave_scelta) +
                    f"\n{self._t('configura_cosa')}"
                )
            return

        # Attesa valore per il parametro selezionato
        if self.stato == "configura_attesa_valore":
            param_key   = self.dati_temp.get("param_key", "")
            param_label = self.dati_temp.get("param_label", param_key)
            salta_key   = self._t("impara_salta")

            if testo.strip().lower() == salta_key.lower():
                # Salta questo parametro
                self._scrivi_risposta(f"  → {param_label} invariato.")
            else:
                valore = testo.strip()
                # Validazione range per valori float noti
                tipo_attuale = type(self._get_nested_config(param_key))
                if tipo_attuale == float:
                    try:
                        v = float(valore)
                        # Range 0-1 per volume
                        if "volume" in param_key and not (0.0 <= v <= 1.0):
                            self._scrivi_risposta(
                                self._t("configura_errore_range",
                                        fallback="Valore fuori range (0.0 – 1.0), riprova."))
                            return
                    except ValueError:
                        self._scrivi_risposta(
                            self._t("configura_errore_tipo",
                                    fallback="Inserisci un numero decimale valido (es. 0.9)."))
                        return
                elif tipo_attuale == int:
                    try:
                        int(valore)
                    except ValueError:
                        self._scrivi_risposta(
                            self._t("configura_errore_tipo",
                                    fallback="Inserisci un numero intero valido (es. 150)."))
                        return
    ...
    def _configura_lista(self):
        """Mostra tutte le chiavi configurabili con il valore attuale."""
        chiavi = self._configura_chiavi_piatte()
        msg = self._t("configura_lista") + "\n"
        for chiave, valore in chiavi:
            msg += f"\n  • {chiave} = {valore!r}"
        self._scrivi_risposta(msg)

    def _configura_parametro_singolo(self, parametro: str):
        """Configura direttamente un singolo parametro passato come argomento."""
        # Cerca corrispondenza tra le chiavi disponibili
        chiavi = self._configura_chiavi_piatte()
        trovata = None
        for chiave, _ in chiavi:
            if parametro == chiave.lower() or chiave.lower().endswith(parametro):
                trovata = chiave
                break
        if trovata is None:
            self._scrivi_risposta(
    ...
    def _configura_fine(self):
        ...
        self._configura_queue = []
        self.dati_temp = {}
        # Aggiorna i predicati AIML con eventuali nuovi nomi
        if self._aiml is not None:
            self._aiml.set_predicato("nome_utente", self.cfg.get("nome_utente", ""))
            self._aiml.set_predicato("nome_avatar",  self.cfg.get("nome_avatar",  ""))
    ...
    def _cmd_apri(self, parsed: dict):
        ...
        if not risultati:
            ...
            return
        entry = risultati[0]
        dati  = entry.get("dati", "").split("\n")[0].strip()

        if self._e_link(dati):
            # Caso normale: è un link, apri subito
            self._apri_link_diretto(dati, entry.get("nome", dati), entry)
        else:
            # Non è un link: suggerisci di mostrare il dato
            self.stato = "attesa_conferma_apri"
            self.dati_temp = {"entry": entry, "dati": dati}
            self._scrivi_risposta(
                self._t("apri_trovato_dato",
                        nome=entry.get("nome", q), dati=dati))
            self._mostra_avatar(entry.get("avatar") or avatar_random(self.cfg))

    def _conferma_apri(self, testo: str):
        """Risposta alla domanda 'vuoi vedere il dato (non apribile)?'"""
        self.stato = "idle"
        entry = self.dati_temp.get("entry", {})
        dati  = self.dati_temp.get("dati", "")
        self.dati_temp = {}
    ...
    def _cmd_dammi(self, parsed: dict):
        ...
        if len(risultati) == 1:
            ...
            if self._e_link(primo_dato):
                ...
                self._scrivi_risposta(
                    self._t("dammi_trovato_link", nome=entry.get("nome", "")))
                self._mostra_avatar(entry.get("avatar") or avatar_random(self.cfg))
            else:
                # Dato normale: mostra subito
                self._scrivi_risposta(
                    self._t("dammi_risultato_1",
                            nome=entry.get("nome",""),
                            soggetto=entry.get("soggetto","")) +
    ...
    def _conferma_dammi(self, testo: str):
        """Risposta alla domanda 'ho trovato un link, vuoi aprirlo?'"""
        self.stato = "idle"
        entry = self.dati_temp.get("entry", {})
        link  = self.dati_temp.get("link", "")
        self.dati_temp = {}
        if testo.strip().lower() in self._t("conferma_si"):
            self._apri_link_diretto(link, entry.get("nome", link), entry)
        else:
            # Mostra comunque il dato testuale
            dati = entry.get("dati", "")
            self._scrivi_risposta(
                self._t("dammi_risultato_1",
                        nome=entry.get("nome",""),
    ...
    _MEDIA_TIPI = {
        # IT
        "immagine":  ((".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"), "immagini"),
        "immagini":  ((".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"), "immagini"),
        "foto":      ((".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"), "immagini"),
        "musica":    ((".mp3", ".wav", ".ogg", ".flac", ".aac"),           "musica"),
        "audio":     ((".mp3", ".wav", ".ogg", ".flac", ".aac"),           "musica"),
        "canzone":   ((".mp3", ".wav", ".ogg", ".flac", ".aac"),           "musica"),
        "canzoni":   ((".mp3", ".wav", ".ogg", ".flac", ".aac"),           "musica"),
        "video":     ((".mp4", ".avi", ".mov", ".mkv", ".webm"),           "video"),
        "film":      ((".mp4", ".avi", ".mov", ".mkv", ".webm"),           "video"),
        # EN aliases
        "image":     ((".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"), "immagini"),
        "images":    ((".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"), "immagini"),
        "music":     ((".mp3", ".wav", ".ogg", ".flac", ".aac"),           "musica"),
        "song":      ((".mp3", ".wav", ".ogg", ".flac", ".aac"),           "musica"),
        "songs":     ((".mp3", ".wav", ".ogg", ".flac", ".aac"),           "musica"),
    }

    def _cerca_asset_per_tipo(self, estensioni: tuple) -> list:
        """Scansiona ASSET_DIR e restituisce i file con le estensioni indicate."""
        cartella = ASSET_DIR.parent   # _dati/asset/  (un livello sopra avatar/)
        if not cartella.exists():
            return []
        return sorted(
    ...
    def _cmd_cerca(self, parsed: dict):
        self._aggiorna_sorgente("memoria")
        q = parsed.get("nome", "").strip().lower()
        if not q:
            self._scrivi_risposta(self._t("cerca_cosa"))
            return

        # Controlla se la query è una parola chiave media
        if q in self._MEDIA_TIPI:
            estensioni, tipo_key = self._MEDIA_TIPI[q]
            tipo_label = self._t(f"cerca_media_{tipo_key}")
    ...
    def _cmd_media(self, parsed: dict):
        """Comando 'media [immagine|musica|video]' — alias breve per cerca media."""
        q = parsed.get("nome", "").strip().lower()
        if not q:
            # Senza argomento mostra il menu dei tipi disponibili
            tipi = "immagine · musica · video"
            self._scrivi_risposta(f"Cosa cerco? Digita: media {tipi}")
            return
        if q in self._MEDIA_TIPI:
            estensioni, tipo_key = self._MEDIA_TIPI[q]
            tipo_label = self._t(f"cerca_media_{tipo_key}")
    ...
    def _avvia_cerca_media(self, estensioni: tuple, tipo_label: str):
        """Cerca file media in asset/ e chiede conferma per la lista."""
        files = self._cerca_asset_per_tipo(estensioni)
        if not files:
            self._scrivi_risposta(
                self._t("cerca_media_vuoto", tipo=tipo_label))
            self._mostra_avatar("triste")
            return
        self.stato = "attesa_conferma_media"
        self.dati_temp = {"media_files": files, "media_tipo": tipo_label}
        self._scrivi_risposta(
            self._t("cerca_media_trovati", n=len(files), tipo=tipo_label))
        self._mostra_avatar(avatar_random(self.cfg))

    def _conferma_media(self, testo: str):
        """Mostra la lista file media se l'utente conferma."""
        self.stato = "idle"
        files      = self.dati_temp.get("media_files", [])
        tipo_label = self.dati_temp.get("media_tipo", "")
        self.dati_temp = {}
        if testo.strip().lower() in self._t("cerca_media_lista_si"):
            msg = self._t("cerca_media_lista", n=len(files), tipo=tipo_label)
    ...
    def _cmd_lingua(self, parsed: dict):
        ...
        salva_config(self.cfg)   
        # Ricarica AIML per la nuova lingua
        if self._aiml is not None:
            self._aiml.scarica_tutto()
        self._inizializza_aiml()
        self.root.title(self._t("titolo_finestra", nome_avatar=self.cfg["nome_avatar"]))
